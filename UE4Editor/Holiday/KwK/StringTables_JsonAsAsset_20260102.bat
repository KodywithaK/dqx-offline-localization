...
: Copy Fmodel's json exports of Dragon Quest X Offline StringTables to this directory
...
FOR %%L IN (!TargetLanguage!) DO (
	...
	FOR /F "usebackq delims=" %%F IN (`dir "%~dp0StringTables\Game\" /s /b /a-d /o:n`) DO (
		SET FILENAME=%%~nF
...
(
	...
	ECHO Splitting Game.locres.yaml by %*
	...
	yq ".[]" -s "key" "%~dp0temp\%*\_%*.json"
	del "%~dp0temp\%*\_%*.json"

	ECHO Formatting *.json for JsonAsAsset imports
	FOR /F "usebackq delims=" %%N IN (`dir "%~dp0temp\%*\" /b /o:n`) DO (
		jq "[{ StringTable: { KeysToEntries: . } }]" "%~dp0temp\%*\%%N" > "%~dp0temp\%*\%%N.tmp"
		ECHO         move "%~dp0temp\%*\%%N.tmp" "%~dp0temp\%*\%%N"
		move "%~dp0temp\%*\%%N.tmp" "%~dp0temp\%*\%%N"
	)
	EXIT /B
)
...
(
	...
	IF !ERRORLEVEL! == 0 (
		...
		ECHO !SOURCE_PATH! >> %~dp0StringTables_JsonAsAsset_20260102_missing_inputs.log
	) ELSE (
		ECHO jq exit-status = !ERRORLEVEL!
	)
	EXIT /B
